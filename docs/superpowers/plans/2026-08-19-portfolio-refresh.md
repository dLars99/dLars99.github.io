...
**Execution note:** Step 1's check found 9 files still importing Panda that the plan's task list never accounted for. Two categories:
1. `src/layouts/Main.astro` — dead code, unreferenced since Task 17 deleted its only consumer (the old `index.astro`). Deleted, plus `src/index.css` (the Panda `@layer` declaration file, only imported by the two `react-routing-tabs` pages, doing nothing useful once nothing populates those layers).
2. `src/components/Projects/Demos/{Portfolio,Greener,Rrt}.astro` (used live by the new `/projects/[slug]` pages via `demoMap`) and `src/components/RrtDemo/{DemoTabs.tsx,Tabpanels/Links.tsx,Tabpanels/TabpanelLayout.tsx}` plus the two `react-routing-tabs` page files (used by the demo the spec calls "unchanged") — both migrated from Panda to plain CSS (scoped `<style>` for `.astro` files, CSS Modules for the `.tsx` files, matching the pattern used everywhere else in this plan). This is a real conflict between two of the plan's own constraints — "Panda CSS is removed entirely" vs. "react-routing-tabs demo pages... unchanged" — that the plan didn't anticipate. Resolved with the user: "unchanged" means the demo's UX/behavior, not literally keeping Panda as an implementation detail after it's gone everywhere else. The RrtDemo migration preserves the original hardcoded hex values (`#007699`/`#004566` for links) rather than switching to the new site's `--color-accent` token, since these pages render standalone (not wrapped in `BaseLayout`) and never load `tokens.css` — a pure mechanical migration with zero visual change, not a reskin.
...
Expected: no matches. If anything shows up, go fix that file first — it was missed in an earlier task. (In execution, several were — see the execution note above; each was resolved before proceeding to Step 2.)

- [ ] **Step 2: Delete the files**

```bash
git rm panda.config.ts textStyles.ts postcss.config.cjs
```

`styled-system/` is gitignored generated output (regenerated by `panda codegen` on every install), not tracked by git — remove it directly with `rm -rf styled-system` instead of `git rm`. Also remove its now-dead `## Panda` block from `.gitignore` (`styled-system`, `styled-system-studio`).
